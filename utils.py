...
def safe_http_error(status_code: int, public_msg: str, internal_error: Exception = None) -> HTTPException:
    """
    Return an HTTPException that shows a generic message to the client in production
    but logs the real error server-side.
    In development mode the real error is included in the response for easier debugging.
    """
    if internal_error is not None:
        app_logger.error(f"{public_msg}: {str(internal_error)}", exc_info=internal_error)

    detail = (
        f"{public_msg}: {str(internal_error)}"
        if settings.DEBUG and internal_error is not None
        else public_msg
    )
